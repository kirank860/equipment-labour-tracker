...
export default function AdminLabour() {
  ...
  const exportToCSV = async () => {
    try {
      const headers = ['Date', 'Job Number', 'Job Name', 'Supplier', 'Employee Name', 'Designation', 'Hours', 'Status', 'Foreman', 'Remarks'];
      const rows = entries.map(e => [
        ...
        e.employee_name || '',
        e.labour_designations?.designation_name || '',
      ...
      if (Platform.OS === 'web') {
        ...
        if (canShare) {
          await Sharing.shareAsync(fileUri, {
            mimeType: 'text/csv',
            dialogTitle: 'Export Labour Entries',
          });
        } else {
  ...
  return (
    <View className={`flex-1 bg-slate-50 ${isMobile ? 'p-4' : 'p-8'}`}>
        ...
        <View className="flex-1 bg-black/50 justify-center items-center p-8">
          <View className="bg-white w-full max-w-md rounded-2xl p-6 shadow-xl">
            ...
            <TextInput
              ...
              placeholder="e.g. Needs more details, incorrect hours..."
      ...
      <View className={`flex-row justify-between items-center mb-8 ${isMobile ? 'flex-wrap gap-y-4' : ''}`}>
        <View>
          <Text className="text-slate-900 text-3xl font-black tracking-tight">Labour Entries</Text>
          <Text className="text-slate-500 text-base mt-2">Manage and review daily labour logs.</Text>
        </View>
        
        <TouchableOpacity 
          onPress={exportToCSV}
          className="flex-row items-center bg-[#166534] px-5 py-3 rounded-xl active:bg-green-800 shadow-sm"
        >
                      ...
                      <View className="flex-col">
                        ...
                        <View className="flex-row justify-between mb-3 border-b border-slate-100 pb-3">
                          ...
                          <View className="items-end">
                            ...
                            <Text className="text-slate-900 font-black text-lg">{entry.total_working_hours}</Text>
                          </View>
                        </View>

                        <View className="mb-4">
                          <Text className="text-xs font-bold text-slate-400 mb-1 uppercase">Labour Details</Text>
            ...
            {selectedEntry && (
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
                ...
                <View className="flex-row justify-between mb-6">
                  ...
                  <View className="flex-1 bg-emerald-50 p-4 rounded-2xl border border-emerald-100 ml-2">
                    <Text className="text-xs font-bold text-emerald-600 uppercase mb-1">Total</Text>
                    <Text className="text-emerald-700 font-black text-2xl">{selectedEntry.total_working_hours} hr</Text>
                  </View>
                </View>
